int x, y, w, h, speedX, speedY; //these are the variable for the ball
int paddleXL, paddleYL, paddleW, paddleH, paddleS;
//sets the paddle location and other things that affect the paddle
//like moving out of the game

boolean upL, downL; //allows game to interact with buttons

color colorL = color(255, 0, 0); //colours the paddle

int scoreL = 0; //these are for the score counts
...
void contactPaddle() {
  ...
  void gameOver() {
    if (x < 0 + w/2) {
      gameOverPage("yougetnothingyoulosegooddaysissr!", colorL);
    }
  }
  ...
  void gameOverPage(String text, color c) {
    ...
    if (mousePressed) {
      setup();
      //this sets the score back to zero
      scoreL = 0;
      //this sets the ball speed to a 4
      speedX = 4;
      speedY = 4;
    }
  }
